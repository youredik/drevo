...
export function parsePersonsCsvString(content: string): Map<number, Person> {
  ...
  for (const line of lines) {
    ...
    const person: Person = {
      ...
      // Column order matches the source Android app (DrevoRoom):
      //   6 = address (где жил),  7 = birthDay,
      //   8 = birthPlace (где родился), 9 = deathDay,
      //  10 = deathPlace (где умер).
      address: fields[6]?.trim() || "",
      birthDay: fields[7]?.trim() || "",
      birthPlace: fields[8]?.trim() || "",
      deathDay: fields[9]?.trim() || "",
      deathPlace: fields[10]?.trim() || "",
